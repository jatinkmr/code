...
// const STPadServerLibCommons = require('./STPadServerLibCommons');
//const STPadServerLibDefault = require('./STPadServerLibDefaults');
var x;
var y;
var p;
...
const convertBase64ToImageAsync = async (base64String) => {
  const image = await Jimp.read(base64String);
  return image;
};
...
app.use(async (req, res, next) => {
  try {
    ...
    function onOpen(connection) {
      ...
      STPadServerLibDefault.handleConfirmSignature = async function () {
        try {
          if (signatureQueue.length === 0) {
            ...
            var a = await STPadServerLibDefault.confirmSignature();
            console.log(a);
            ...
            var b = await STPadServerLibDefault.getSignatureImage(params);
            const base64 = b.file;

            const filePath = "./output-image.png";
            fs.writeFileSync(filePath, base64, "base64");
            console.log("Image has been saved to:", filePath);
      ...
      // STPadServerLibCommons.handleNextSignaturePoint = async function (x, y, p) {
      //   //console.log(x, y, p);
      //   res.locals.signatureData=await STPadServerLibDefault.getSignatureData(getSignatureDataParams);
      // };
      const signatureQueue = []; // Queue to store signature points

      const processSignatureQueue = async () => {
        while (signatureQueue.length > 0) {
          const { x, y, p } = signatureQueue.shift(); // Dequeue the next signature point
          try {
            signatureData = await STPadServerLibDefault.getSignatureData(
            ...
            res
              .status(500)
              .send("Error while getting signature data: " + error.message);
          }
        }
      };
      ...
      STPadServerLibCommons.handleNextSignaturePoint = async function (
        ...
        signaturePoint = { x: x, y: y, p: p };
        signatureQueue.push({ x, y, p }); // Enqueue the signature point
        io.emit("signaturePoint", signaturePoint);
        console.log(signaturePoint);
       // await processSignatureQueue(); // Process the queue
      };
      ...
      async function performOperations(
        ...
        try {
          ...
          //  const result4 = await STPadServerLibDefault.stopSignature();
          console.log(result1, result2, result3);
          // Handle the results of each operation here if needed.
        } catch (error) {
          console.error(error);
        }
      }
...
app.get("/openConnection", async (req, res) => {
  // const waitForSignatureData = () => {
  //   return new Promise((resolve) => {
  //     const checkSignatureData = () => {
  //       if (res.locals.signatureData) {
  //         resolve(res.locals.signatureData);
  //       } else {
  //         setTimeout(checkSignatureData, 1); // Check again in 100 milliseconds
  //       }
  //     };
  //     checkSignatureData();
  //   });
  ...
  // try {
  //   const signatureData = await waitForSignatureData();

  //   if (signatureData) {
  //     console.log(signatureData);
  //     res.json({ signatureData });
  //   } else {
  //     res.status(500).send("Signature data not available");
  //   }
  // } catch (error) {
  //   console.error(error);
  //   res.status(500).send("Error: " + error.message);
  ...
  // console.log(res.locals.signatureData,res.locals.x,res.locals.y,res.locals.p);
  res.render("canvas.ejs");
});
//})
//var params = new STPadServerLibDefault.Params.openPad(1); STPadServerLibDefault.openPad(params) .then(function(value) { alert("Pad opened; samplingRate: " + value.samplingRate); }, function(reason) { alert(reason.errorMessage); });
