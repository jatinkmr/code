...
app.use(async (req, res, next) => {
    try {
        ...
        function onOpen(connection) {
            ...
            console.log('getSignatureDataParams :- ', getSignatureDataParams)
            ...
            STPadServerLibDefault.handleCancelSignature = async function () {
                console.log('canceling the signaturePad...')
            ...
            async function performOperations(
                ...
                try {
                    ...
                    console.log(`result1 :- ${JSON.stringify(result1)}, result2 :- ${JSON.stringify(result2)} and result3 :- ${JSON.stringify(result3)}`)
                } catch (error) {
                    console.error(error)
                }
            }
            ...
            // STPadServerLibDefault.searchForPads(params1).then(function (searchPadValue) {
            //     console.log('searchPadValue :- ', searchPadValue)
            //     STPadServerLibDefault.openPad(params2).then(function (padValue) {
            //         console.log('padValue :- ', padValue)
            //         STPadServerLibDefault.startSignature(
            //             params3,
            //             STPadServerLibDefault.handleCancelSignature,
            //             STPadServerLibDefault.handleRetrySignature,
            //             STPadServerLibDefault.handleConfirmSignature,
        ...
        function onClose(connection) {
            console.log("Signotec Sigma Pad disconnected/close", connection)
        }

        function onError(error) {
            console.log("Unale to connect with Signotech Sigma Pad facing some kind of error", error)
        }

        process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
        // "local.signotecwebsocket.de", "localhost" or "127.0.0.1"
        ...
        console.log('Error at middleware function :- ', error)
    }
    next()
})
